    ...
    IMerlinchainBoxRewards
{
    ...
    /**
     * @dev Update token quantity charged
     * @param tokenQuantityChargedNumber 1 is MUSDT else MBTC
     * @param chargedNumber New quantity charged
     */
    function updateTokenQuantityCharged(uint256 tokenQuantityChargedNumber, uint256 chargedNumber) external onlyOwner {
        MerlinchainDBALRewardsStorage storage $ = _getMerlinchainDBALRewardsStorage();

        if (tokenQuantityChargedNumber == 1) {
            $.MBTCQuantityCharged = uint128(chargedNumber);
        } else {
            $.MUSDTQuantityCharged = uint128(chargedNumber);
        }

        emit SetTokenQuantityCharged(tokenQuantityChargedNumber, chargedNumber);
    }
